...
import {
  DEPLOYMENT_STATUS_ICON_PROPS,
  ENV_REGEX,
  ENV_TAG_PROPS,
  GITHUB_URL,
  LOG_SEVERITY_TAG_PROPS,
  PRICES,
  RAILWAY_API_URL,
  RAILWAY_URL,
  SEVERITY_REGEX,
...
export function deploymentPage(
  ...
  serviceId: string,
  deploymentId: string,
  page: "details" | "deploy" | "build" = "details",
) {
...
export function calculateTotalCostByProjectId(usage: RawUsage) {
  ...
  return Object.keys(usageByProjectId).reduce(
    (acc, key) => {
  ...
  return usage.reduce(
    (acc, curr) => {
      ...
      switch (curr.measurement) {
        case "CPU_USAGE":
          acc[projectId].cpu += curr.value || curr.estimatedValue || 0;
          break;
        case "NETWORK_TX_GB":
          acc[projectId].network += curr.value || curr.estimatedValue || 0;
          break;
        case "MEMORY_USAGE_GB":
          acc[projectId].memory += curr.value || curr.estimatedValue || 0;
          break;
        case "DISK_USAGE_GB":
          acc[projectId].disk += curr.value || curr.estimatedValue || 0;
          break;
        case "EPHEMERAL_DISK_USAGE_GB":
          acc[projectId].disk += curr.value || curr.estimatedValue || 0;
          break;
      }
...
export function calculateTotalCost(usage: Record<Measuements, number>) {
  return Object.keys(usage).reduce((acc, key) => (acc += usage[key as Measuements] * PRICES[key as Measuements]), 0);
}
