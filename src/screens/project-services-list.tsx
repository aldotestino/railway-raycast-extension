...
function ProjectServices({ projectId }: { projectId: string }) {
  ...
  const filteredServices = data.services.filter((service) => environment !== "all" ? service.environment === environment : true);
    ...
    >
      ...
      {filteredServices.map(service => (
        <List.Item
          ...
          accessories={[
            { tag: { value: service.environment, color: getEnvironmentTag(service.environment).color || Color.PrimaryText }, icon: getEnvironmentTag(service.environment).icon },
          ]}
          actions={
            <ServiceActions
              service={service}
