...
function ServiceActions({ service, projectId }: { service: Service; projectId: string }) {
  return (
    <ActionPanel>
      <ActionPanel.Section title="Open in Raycast">
        <Action.Push
          title="Show Deployments"
          target={<ServiceDeployments projectId={projectId} serviceId={service.id} />}
        />
      </ActionPanel.Section>
      ...
      <ActionPanel.Section title="Open in Browser">
        <Action.OpenInBrowser
          title="Open Service"
          url={servicePage(projectId, service.id)}
          shortcut={{ modifiers: ["opt"], key: "s" }}
        />
        {service.domain && (
          <Action.OpenInBrowser title="Open Domain" url={service.domain} shortcut={{ modifiers: ["opt"], key: "w" }} />
        )}
        {service.repo && (
          <Action.OpenInBrowser
            title="Open GitHub"
            url={repoPage(service.repo)}
